...
try {
$ekle				= $db->prepare("INSERT INTO urunler SET baslik=?, aciklama = ?,sira=?,kategori_id=?,min_adet=?,max_adet=?,tutar=?,baglanti_url_check=?,baglanti_value=?,api_id=?,api_servis=? ");
$ekle->execute(array($baslik, $aciklama, $sira,$kategori_id,$min_adet,$max_adet,$tutar,$baglanti_url_check,$baglanti_value,$api_id,$api_servis));
}catch(PDOException $e){
die($fonk->hata("Bir hata oluştu eklenemiyor!"));
}

$fonk->tamam("Başarıyla Eklendi!");
$fonk->yonlendir("index.php?do=urunler",2000);